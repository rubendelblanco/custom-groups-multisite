...
class CGMBloquesTable extends WP_List_Table
{
    /**
    ...
    function column_nombre($item)
    {
        ...
        $actions = array(
            'edit' => sprintf('<a href="?page=bloques&action=edit&id=%s">%s</a>', $item['id'], __('Editar', 'grupos')),
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'id' => __('ID','custom_groups'),
            'nombre'  => __('Nombre','custom_groups'),
            'enlaces' => __('Nº de enlaces', 'reparaciones')
        );
        return $columns;
    }
    ...
    {
        global $wpdb;
        $table_bloques = $wpdb->base_prefix.'cgm_bloques';
        ...
        $total_items = $wpdb->get_var("SELECT COUNT(id) FROM $table_bloques");
        ...
        if (isset( $_REQUEST ["s"] )){
         ...
         //Consulta mejorable
        $query = "SELECT {$table_bloques}.id, {$table_bloques}.nombre FROM {$table_bloques} WHERE 1";
